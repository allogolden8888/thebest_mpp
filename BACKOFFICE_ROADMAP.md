...
2. **Секреты и обязательная конфигурация не подключены — механизм startup-секретов и per-operator webhook credentials закрыты 2026-09-11 (см. «Secrets/config lifecycle» в журнале ниже), реальные production-значения по-прежнему не провизионированы.** `partner-smpp-gateway` читает `PARTNER_CONFIG_PATH`, но K8s монтирует конфиг только двум другим сервисам — под фактически неработоспособен (`k8s/generate_manifests.py:33`, `partner-smpp-gateway/.../Main.java:63`).
...
8. **Обязательные startup-секреты доведены до pod env.** Terraform принимает PEM только через sensitive variables и записывает раздельные Vault KV entry: backoffice RSA keypair, partner IdP verification key. External Secrets создаёт соответствующие Kubernetes Secrets, а deployment-каталог подключает их ровно к `backoffice-api` и partner/self-service/compliance API. Backoffice и partner IdP намеренно остаются разными trust domain. **ОБНОВЛЕНО 2026-09-11:** единый `operator webhook token` (общий Vault KV entry/k8s Secret на всех операторов) убран из этого списка целиком — `operator-http-gateway` больше не получает статический секрет через `envFrom`, см. «Secrets/config lifecycle» ниже для per-operator механизма, который его заменил.
...
- **Secrets/config lifecycle — оба P0#1 подпункта механически закрыты 2026-09-11, реальные секреты всё ещё не провизионированы.** ✅ **JWKS/`kid` rotation** (commit `d3987e0`): `backoffice-api` больше не проверяет один статический public key — `TokenIssuer` штампует `kid` в заголовок JWT, `GET /v1/.well-known/jwks.json` отдаёт текущий ключ + 0+ предыдущих из `JWT_PREVIOUS_PUBLIC_KEYS_PEM` (grace window на время ротации), валидатор резолвит ключ проверки по `kid` токена, не по хардкоду. ✅ **Per-operator webhook credentials** (заменяет общий `WEBHOOK_AUTH_TOKEN` на всех операторов сразу): `operator-http-gateway` теперь резолвит `/webhook/dlr/{operator_id}` через `internal/webhookauth` — `credential_ref` из `operator.schema.json` `http_profile.webhook_auth` (уже существовавшее поле, схема дополнительно требует его для `BEARER_TOKEN`/`HMAC_SIGNATURE`) читается из Configuration Redis (`internal/opconfig`, тот же паттерн, что `billing-service::TariffCache`), затем секрет — из Vault по Kubernetes auth (`internal/vault`, зеркало `credential-issuer-service`/`partner-rest-receiver::VaultAuthVerifier`), с TTL-кешем и constant-time сравнением (`iam-service::dummyHash`-паттерн против утечки существования оператора по времени ответа). Terraform: `vault_kv_secret_v2.operator_webhook`/`variable "operator_webhook_auth_token"` (bulk-секрет на всех операторов) удалены, добавлены `vault_policy.read_operator_webhook_credentials` + `vault_kubernetes_auth_backend_role.operator_webhook_credential_readers` (read-only `mpp/data/operators/*`, тот же паттерн, что `partner_credential_readers`). **Общая для обоих оговорка, как и раньше:** сами production-значения (JWT keypair, per-operator webhook секреты) всё ещё должны быть безопасно переданы в Terraform/bootstrap и провизионированы — для операторов это теперь ручной `vault kv put mpp/operators/<id> ...` (нет ротации/self-service RPC уровня `credential-issuer-service`, тот умеет выпускать только PARTNER credentials). Текущий `partner.valid.json` — пример, а не production source of truth; в нём нет ни одного `SMPP_BIND`, поэтому один только mount не делает partner SMPP готовым.
