...
| 3.5 | ✅ Message State Resolver транзакционная гарантия — Главный агент | Транзакционный `KafkaProducer` (HLD §10.1) реализован; `restore-from-changelog` тоже реализован и закоммичен (commit `41d4f04`, PART 2 кодревью) — `KafkaIo.restoreFromChangelog` вызывается из `Main.java` до `health.ready.set(true)`, читает `message-state.changelog` целиком до end-offset'ов и материализует в `MessageStateStore` перед стартом обработки живого трафика; корректность после рестарта пода подтверждена. Эта строка ранее ошибочно значилась открытой — устаревшая заметка, не отражавшая уже сделанную работу. **Остаточный пункт, не блокирующий:** `MessageStateStore` — не RocksDB, а `ConcurrentHashMap`, так что рестарт при большом объёме changelog означает полный реплей топика (латентность старта, не потеря корректности) — RocksDB имело бы смысл только как перф-оптимизация restart-latency, не как исправление гарантии |
...
5. **Docker/PostgreSQL/Kafka — реальные приколы этой сессии, разойдутся по независимым песочницам агентов:**
   ...
   - **Обновление 2026-08-06: Docker Desktop стал доступен у Главного агента.** `docker build` реально прогнан для всех 32 сервисов (пользователь запускал сами сборки напрямую в терминале — сборки внутри агентской песочницы совпадали по времени с необъяснимыми обрывами сети/VPN на машине пользователя, причина не установлена, поэтому дальнейшие `docker build` этой сессией не запускаются агентом напрямую). Найдены и исправлены 4 реальных бага, невидимых при статичном чтении Dockerfile: 5 Rust-сервисов собирались на `rust:1.82-bookworm`, но `Cargo.toml` использует `edition = "2024"` (нужен Cargo ≥1.85) — поднято до `rust:1.90-bookworm`; тем же 5 сервисам не хватало `libprotobuf-dev` (`protobuf-compiler` даёт только сам бинарник `protoc`, well-known types типа `timestamp.proto` — в отдельном пакете, отключённом `--no-install-recommends`); `delivery-service` жёстко прописывал `grpc.plugin.classifier=linux-x86_64`, ломался на Apple Silicon (Docker Desktop собирает нативный `arm64`-контейнер по умолчанию) — теперь определяется через `uname -m`; `backoffice-ui` (`node:24-bookworm-slim`) не нёс пакет `ca-certificates` вообще. `infra/docker/README.md` — опциональный extra CA-сертификат для TLS-инспектирующего прокси этой сети, opt-in через `docker build --secret`, ничего не меняет в обычной сборке без него.
...
| 2.3 | 🟡 частично — `docker build` для всех 32 сервисов реально прогнан и провалидирован (2026-08-06, см. "Координация" п.5), 4 реальных бага найдены и исправлены. **Остаточное:** разворот в staging-кластер (нет `kind`/`minikube` в этом окружении) и first-run диагностика (readiness/liveness на `/healthz`/`/readyz`) — не начато | — |
...
Все 11 сервисов Главного агента из "ходового скелета" реализованы. `docker build` самого образа теперь реально проверен для всех (2026-08-06). Всё ещё не проверено: реальный Kafka-брокер (нет `kind`/`minikube` в этом окружении для полноценного staging).
...
| # | Задача | Источник flag |
|---|---|---|
| 5.1 | 🟡 частично — `routing.number_range` по Узбекистану: 9 подтверждённых в чате префиксов + 4 добавленных по общему знанию нумерации (33 Beeline; 95/97/88 Uzmobile), только для операторов с реальным connection-профилем в платформе. Остаточное: полевая проверка новых 4 на реальных номерах, остальные узбекистанские операторы (Perfectum отдельным брендом, Mobiuz и т.д.) — не добавлены, у них нет connection-профиля | `migrations/README.md`, `data_infrastructure_spec.md` §1.9a |
