...
|---|---|---|---|
| 2.1 | Реализация каждого сервиса из LLD-методов (`service_internal_methods.md`) на своём языке (Rust/Java/Go) — это большая часть "написания кода" проекта, вне скоупа документов | 🟡 6/32 готово (Главный агент: `destination-resolution-service`, `policy-service`, `billing-service`, `routing-service`, `pipeline-engine`, `partner-rest-receiver`), 26 распределены — см. "Распределение между агентами" выше | Фаза 3 |
...
Не проверено ни у одного из шести сервисов: реальный Kafka-брокер (нет `docker`/`kind` в этом окружении), `docker build` самого образа — см. README каждого сервиса.
