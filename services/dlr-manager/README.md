...
* **Poison-message (невалидный protobuf, либо любая другая ошибка обработки, на любом из двух входных топиков) не продвигает оффсет** — раньше это было верно только пока следующая запись того же poll'а не окажется на другой партиции; поскольку коммит был per-record, а не per-partition-batch, успешная запись на ТОЙ ЖЕ партиции сразу после поломанной коммитила мимо неё, и poison-запись терялась молча и навсегда (`CODE_REVIEW.md` PART 2, dlr-manager #2). Исправлено `internal/kafkaio/OffsetTracker` — тот же паттерн, что `KafkaIo.OffsetTracker` в delivery-service/billing-service (Java): создаётся заново на каждый poll, приостанавливает партицию при первой ошибке, коммитит только то, что обработано успешно ДО точки остановки. Теперь партиция реально "застревает" на проблемной записи (растущий consumer lag — сигнал для алертинга Фазы 6), а не молча теряет её.
