...
///
/// Здесь тот же паттерн и КРАТНО большая конкурентность: `ROUTING_CONCURRENCY` по
...
pub fn producer_pool_size() -> usize {
    std::env::var("ROUTING_PRODUCER_POOL_SIZE").ok().and_then(|v| v.parse().ok()).filter(|v| *v > 0).unwrap_or(4)
}
