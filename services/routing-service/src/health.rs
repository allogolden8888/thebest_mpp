...
//! остальных сервисов этого среза (см. destination-resolution-service/src/health.rs).
...
pub fn router(state: Arc<HealthState>) -> Router {
    Router::new()
                ...
                move || {
                    ...
                    async move {
                        if state.ready.load(Ordering::Relaxed) {
                            ...
                            (axum::http::StatusCode::SERVICE_UNAVAILABLE, "route table not loaded")
                        }
                    }
                }
            }),
        ...
        .route(
            "/metrics",
            get(|| async {
                "# HELP routing_service_up Service liveness placeholder\n# TYPE routing_service_up gauge\nrouting_service_up 1\n"
            }),
        )
}
