...
async fn process_one_completed_record(
    ...
    let mut state = match store.load(&event.message_id).await {
        Ok(Some(s)) => s,
        Ok(None) => {
            // ИЗМЕРЕНО 2026-09-04: раньше здесь был `return false` — офсет НЕ
            // коммитился в расчёте на переобработку. Но отсутствие состояния
            // здесь не временный сбой: повтор не может помочь никогда, потому
            // что состояние уже не вернётся. Штатных причин три, и все
            // регулярные:
            //   * дубликат события (Kafka at-least-once — норма, не аномалия);
            //   * позднее событие после finalize_pipeline (что признаёт и сам
            //     прежний текст сообщения — "либо уже финализировано");
            //   * истёкший TTL-бэкстоп exec:{message_id} (~25ч).
            //
            // Цена прежнего поведения — не лишний лог, а полная остановка
            // продвижения: OffsetTracker двигает коммит только сплошным
            // watermark'ом, поэтому ОДНА такая запись пиннит офсет навсегда, а
            // цикл переобрабатывает её бесконечно. На живой системе это и
            // наблюдалось: stage.completed застрял на офсете 864133 при
            // LOG-END 995208 — лаг 131075 не двигался, при том что все новые
            // сообщения обрабатывались нормально. То есть дефект тихий: он не
            // ломает доставку, но навсегда останавливает коммит и жжёт CPU
            // фоновым переперемалыванием, отравляя заодно любые замеры
            // производительности.
            //
            // Семантика та же, что у уже существующего Decision::Ignored
            // (устаревшее/дублирующееся событие — состояние не трогаем, офсет
            // безопасно двигаем), поэтому и обрабатываем так же.
            tracing::warn!(
                "нет ExecutionState в Runtime Redis для message_id={} — событие пропущено (дубликат, позднее после финализации или истёкший TTL); офсет продвигаем",
                event.message_id
            );
            return true;
        }
