// Тот же паттерн, что services/destination-resolution-service/build.rs —
// компилирует настоящие platform-contracts/*.proto через prost-build,
// PLATFORM_CONTRACTS_DIR override для Docker (build context — корень репо).
fn main() {
