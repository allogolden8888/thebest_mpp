package uz.mpp.billingledgerwriter.health;
...
public final class HealthServer {
    ...
    public void start() throws IOException {
        ...
        server.createContext("/metrics", exchange -> {
            byte[] body = ("# HELP billing_ledger_writer_up Service liveness placeholder\n"
                + "# TYPE billing_ledger_writer_up gauge\nbilling_ledger_writer_up 1\n").getBytes();
            exchange.sendResponseHeaders(200, body.length);
            exchange.getResponseBody().write(body);
            exchange.close();
        });
