...
public final class KafkaIo {
    ...
    public static KafkaConsumer<String, byte[]> buildConsumer(String bootstrapServers, String groupId) {
        ...
        // 2026-09-03, замер 300 TPS на 8-ядерной машине: цикл ниже —
        // poll -> отдать ВСЮ пачку в пул -> дождаться КАЖДОГО future ->
        // commitSync -> только потом следующий poll. Значит следующий poll
        // не начнётся, пока не дренирована вся пачка, и латентность записи
        // равна её позиции в дренаже, а не времени обработки. При 1000
        // записей на 300 TPS одна пачка — это ~3.3с трафика, и хвост пачки
        // столько и ждёт: измеренный DELIVERY-хоп давал p50=1420мс при
        // ПУСТЫХ очередях ниже по потоку (окно SMPP 0-12 из 100, очередь
        // пейсера 0, диспатч 315/с ровно по входящему потоку) — то есть
        // это чистая задержка батчинга, не нехватка мощности.
        //
        // Пропускная способность ограничена пулом воркеров (BILLING_CONCURRENCY), а не
        // размером пачки: пачки заметно больше пула лишь удлиняют дренаж,
        // ничего не ускоряя. 256 — примерно два «прохода» пула по 128,
        // так что все потоки заняты, а дренаж короче в 4 раза. Цена —
        // чаще commitSync (на 300 TPS ~1.2/с вместо ~0.3/с), это
        // пренебрежимо против выигранных секунд.
        //
        // Настраиваемо через env, чтобы подбирать под железо БЕЗ пересборки
        // образа — тот же мотив, что у BILLING_CONCURRENCY рядом.
    ...
    public static void run(
        ...
        AdaptiveThreadPoolCalibrator calibrator = new AdaptiveThreadPoolCalibrator(
            ...
            // BILLING_CONCURRENCY снова читается. С появлением калибратора эта
            ...
            envInt("BILLING_CONCURRENCY", AdaptiveThreadPoolCalibrator.FLOOR));
        try {
            while (running.get()) {
