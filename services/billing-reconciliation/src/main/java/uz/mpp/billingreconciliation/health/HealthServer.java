package uz.mpp.billingreconciliation.health;
...
public final class HealthServer {
    ...
    public void start() throws IOException {
        ...
        server.createContext("/metrics", exchange -> {
            byte[] body = ("# HELP billing_reconciliation_up Service liveness placeholder\n"
                + "# TYPE billing_reconciliation_up gauge\nbilling_reconciliation_up 1\n").getBytes();
            exchange.sendResponseHeaders(200, body.length);
            exchange.getResponseBody().write(body);
            exchange.close();
        });
