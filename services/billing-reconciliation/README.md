...
| `persist_audit` | `store/ReconciliationAuditStore` | `MainTest` — реальный PostgreSQL, `migrations/V018__billing_reconciliation_audit.sql` (см. "Проверено кодревью" ниже) |
...
* **Сопутствующая находка**: `billing.reconciliation_audit` не была мигрирована (см. старый "Открытый вопрос" ниже, ныне закрыт) — без этой таблицы `persist_audit` физически не мог работать, значит и recompute-then-unfreeze нельзя было бы протестировать end-to-end. Добавлена `migrations/V018__billing_reconciliation_audit.sql` (та же схема, что уже была задокументирована в докстринге `ReconciliationAuditStore` — не выдумана заново), применена и провалидирована на локальном PostgreSQL 17.
