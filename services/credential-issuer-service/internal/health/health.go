...
// внешних зависимостей (Postgres, Vault). Тот же принцип, что у
// backoffice-api: /readyz отражает реальное состояние зависимостей, не
// статический флаг, выставленный один раз при старте.
...
func Router(state *State) *http.ServeMux {
	...
	mux.HandleFunc("/metrics", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte("# HELP credential_issuer_service_up Service liveness placeholder\n# TYPE credential_issuer_service_up gauge\ncredential_issuer_service_up 1\n"))
	})
