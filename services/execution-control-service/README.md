...
## `ApplyOverrideRequest.incident_id` (luminous-hugging-charm.md Ф7, incident-service)

`internal_control.proto` `ApplyOverrideRequest` получил опциональное поле 8, `int64 incident_id` — линкует этот override к инциденту (`incident.incidents.id`, новый `services/incident-service`, `migrations/V028__incident.sql`), без хрупкого join по времени. `0` (proto3 default) означает "без инцидента" — безопасно, `incident.incidents.id` это `BIGSERIAL`, начинается с 1, реальный id никогда не 0 (`grpcserver.ApplyOverride` явно проверяет `!= 0` перед тем, как прокинуть значение дальше как `*int64`, чтобы обычные override без инцидента писали `NULL`, а не `0`). `store.AuditEntry.IncidentID` (`internal/store/audit.go`) прокидывает значение в новую nullable-колонку `control.execution_control_audit.incident_id` (та же миграция) — **без какой-либо валидации против `incident.incidents`**, там нет FK через границу схем (см. `migrations/README.md` V028, `services/incident-service/README.md`), этот сервис не знает и не обязан знать о существовании конкретного `incident_id`, просто пишет то, что прислал вызывающий. `ClearOverrideRequest` НЕ получил такое поле — снятие override не открывает новый контекст инцидента, это симметричное завершение уже начатого разбора. Покрыто `store/audit_test.go` `TestPersistOverrideAuditWithIncidentIDRoundTrips` — реальная вставка настоящей строки `incident.incidents` + `PersistOverrideAudit` с ненулевым `IncidentID` + readback, подтверждающий, что `incident_id` дошёл до Postgres, а не только скомпилировался.
