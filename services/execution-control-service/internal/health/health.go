...
// PodMonitor scrape target). Перенесена по образцу
// services/destination-resolution-service/src/health.rs — та же трёхпутевая
// форма, тот же смысл readyz (503 до готовности, 200 после).
package health
...
func Router(state *State) *http.ServeMux {
	...
	mux.HandleFunc("/metrics", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte("# HELP execution_control_up Service liveness placeholder\n# TYPE execution_control_up gauge\nexecution_control_up 1\n"))
	})
