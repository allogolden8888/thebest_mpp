package uz.mpp.billingoutbox.health;
...
public final class HealthServer {
    ...
    public void start() throws IOException {
        ...
        server.createContext("/metrics", exchange -> {
            byte[] body = ("# HELP billing_outbox_publisher_up Service liveness placeholder\n"
                + "# TYPE billing_outbox_publisher_up gauge\nbilling_outbox_publisher_up 1\n").getBytes();
            exchange.sendResponseHeaders(200, body.length);
            exchange.getResponseBody().write(body);
            exchange.close();
        });
