...
func TestIsPausedFalseByDefault(t *testing.T) {
	...
	if s.IsPaused("BILLING") {
		t.Fatalf("пустой снапшот не должен блокировать republish")
	}
}
...
func TestIsPausedTrueOnlyForMatchingStage(t *testing.T) {
	...
	if s.IsPaused("DELIVERY") {
		t.Fatalf("STAGE=BILLING PAUSED не должен блокировать DELIVERY")
	}
}
...
func TestDeleteTombstoneClearsPause(t *testing.T) {
	s := New()
	s.Apply(commonv1.ExecutionControlScope_EXECUTION_CONTROL_SCOPE_STAGE, "BILLING", commonv1.ExecutionControlState_EXECUTION_CONTROL_STATE_PAUSED)
	s.Delete(commonv1.ExecutionControlScope_EXECUTION_CONTROL_SCOPE_STAGE, "BILLING")

	if s.IsPaused("BILLING") {
		t.Fatalf("tombstone должен снимать запись, не оставлять PAUSED")
	}
}
