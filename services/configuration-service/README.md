...
## Production hardening 2026-09-14: archive теперь публикуется

`ArchiveVersion` раньше только менял `config_versions.status` в PostgreSQL. Новая строка `config_outbox` не возникала, поэтому `config-event-publisher` и ни один full-mirror consumer в data plane вообще не могли узнать об архивировании. Теперь UPDATE и INSERT outbox выполняются в одной транзакции. Повторный archive уже archived-версии отклоняется и не создаёт дубль terminal event. Контракт сохраняет текущую same-version семантику: `active(N) -> archived(N)`; consumers должны считать terminal status доминирующим для той же версии.
