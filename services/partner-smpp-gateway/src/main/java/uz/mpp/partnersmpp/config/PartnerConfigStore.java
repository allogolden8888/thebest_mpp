...
public final class PartnerConfigStore implements AutoCloseable {
            ...
            while (true) {
                for (String key : cursor.getKeys()) {
                    ...
                    try {
                        ...
                        Map<String, SmppBindCredential> partnerCredentials = loadPartnerVersion(
                            commands,
                            partnerId,
                            version
                        );
                        rejectCrossPartnerSystemIdCollision(merged, partnerCredentials);
                        merged.putAll(partnerCredentials);
    ...
    public synchronized boolean applyEvent(String partnerId, long version, String status, byte[] payloadJson) {
        ...
        if (STATUS_ACTIVE.equals(status)) {
            ...
            if (!STATUS_ACTIVE.equals(parsed.status())) {
                throw new IllegalArgumentException("active config.changes содержит неактивный partner payload");
            }
    ...
    private Map<String, SmppBindCredential> loadPartnerVersion(
        ...
        if (payload == null || payload.isEmpty()) {
            LOG.warning(() -> "config:current указывает на версию " + version + ", но config:version пуст для partner_id=" + partnerId);
            return Map.of();
        }
