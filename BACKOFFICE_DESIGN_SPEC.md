...
**Статус: 🔨 строю (~1 день). Данные ЕСТЬ** в Runtime Redis (`operator_route:{operator_id}:{route_id}`).
...
```json
{
  "routes": [
    { "operator_id": "beeline_uz", "route_id": "beeline_smpp_primary",
      ...
      "owning_instance_id": "…", "heartbeat_at": "…", "route_epoch": 4870604671880,
      "healthy": true }
  ]
}
```
**Дизайн:** карточки операторов со светофором (жив/heartbeat протух), кто держит сессию, когда последний heartbeat.
...
**Статус: ✅ API есть** (отдельный сервис `compliance-api`, порт 8083), 🔨 прокси через backoffice-api ~0.5 дня.

| Действие | API |
|---|---|
| Проверить номер | `GET /v1/compliance/consent?msisdn=998901234567` → `{category_blacklist: ["ADVERTISING"], sender_blacklist: ["SPAMMER"]}` |
| Добавить запись вручную | `POST /v1/compliance/consent` (право `compliance:write`) |

**Дизайн:** поиск по номеру → карточка с двумя списками (заблокированные категории, заблокированные отправители) + форма добавления.
