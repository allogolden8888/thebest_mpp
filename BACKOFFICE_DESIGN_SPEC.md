...
**Статус: ✅ есть, обе стороны (`backoffice-ui` и `partner-portal-ui`), живой polling-раунд-трип проверен.** Реальная новая фича с нуля — до этого захода ничего похожего не было нигде в платформе (ни топика, ни таблицы для сообщений админ↔партнёр).

Архитектура: новый выделенный `chat-service` (маленький Go control-plane сервис, тот же класс, что `iam-service`/`incident-service` — gRPC :9000, health :9090) — единственный писатель/читатель `support.chat_messages` (`migrations/V034__chat_messages.sql`, новая схема `support`, один тред на `partner_id`). `backoffice-api` и `partner-self-service-api` — оба тонкие gRPC-прокси поверх `platform-contracts/grpc/chat.proto` `ChatService`, ни один не имеет прямого доступа к таблице. Полный разбор, почему отдельный сервис, а не прямой общий доступ к таблице из двух кодовых баз (единственный найденный в платформе случай двух писателей в реальном времени в одну таблицу) — `services/chat-service/README.md`.

Polling, не websocket (ни одна из 40+ служб платформы не поднимает websocket-инфраструктуру) — TanStack Query `refetchInterval`, первое использование этого механизма в обоих UI. `read_at` — побочный эффект `ListMessages`, вызванного противоположной стороной (не отдельный mark-as-read эндпоинт), единственный источник счётчика "N новых" в сайдбаре `backoffice-ui` (`ListThreads` — список партнёров с непрочитанными; не вызывается `partner-self-service-api`, партнёр всегда читает один свой тред напрямую).

`backoffice-ui`: `ChatView.vue` — список партнёров слева (сайдбар с unread-бейджами) + тред справа, право `chat:write` (выдано `backoffice-admin`/`support-agent`). `partner-portal-ui`: `ChatView.vue` — только тред, без пикера партнёра (partner_id всегда из JWT), открыт любой партнёрской роли (не только `partner-admin` — переписка с поддержкой не деструктивна).

**Граница объёма (сознательная, не спрятанная)**: без read-receipts UI-полировки (read_at только считает unread), без typing indicators, без file attachments — только plain-text сообщения, как показывает design-референс. `since`-курсор полностью поддержан на бэкенде (`ListMessages`), но не используется фронтендом ни в одном из двух UI — каждый poll перезапрашивает тред целиком (объём одного треда поддержки мал, инкрементальный merge курсора поверх TanStack Query усложнил бы код без реальной пользы на этом масштабе).

Проверено живым `docker compose` раунд-трипом (`chat-service`, `backoffice-api`, `partner-self-service-api`, `backoffice-ui`, `partner-portal-ui` — все свежепересобраны и перезапущены), реальными JWT из `infra/docker/dev-secrets/`: админ отправляет → партнёр видит через `GET /v1/self-service/chat/messages`; партнёр отправляет → админ видит через `GET /v1/chat/{partner_id}/messages`; повторный `GET` с `since=<курсор>` возвращает только новое сообщение, не всю историю. `go build`/`go vet`/`go test ./...` зелёные в `chat-service`/`partner-self-service-api`; `npm run build` + `npm test` зелёные в `partner-portal-ui`.
...
**A2P/DLR per-PDU лог: ✅ готово.** Raw per-PDU SMPP-трасса (`submit_sm`/`submit_sm_resp`/`deliver_sm`/`deliver_sm_resp`, каждый реально пересечённый по проводу PDU с `sequence_number`/`status`/`occurred_at`) — новый `OperatorPduLog` (`platform-contracts/events/operator_events.proto`), инструментация `operator-smpp-session-manager` → новый топик `operator.pdu.log` → новый сервис `pdu-log-writer` → `analytics.operator_pdu_log` (ClickHouse) → `GET /v1/messages/{message_id}/pdu-log` (`backoffice-api`) → карточка "Пер-PDU лог" в `MessagesView.vue`. Живьём проверено сквозным реальным сообщением через docker-compose стенд (см. `BACKOFFICE_ROADMAP.md`, раздел "A2P/DLR per-PDU лог" — там же найденные и исправленные по пути пробелы: топик и сервис не были заведены в локальном стенде). Известное ограничение, оставленное как есть: `deliver_sm_resp` — чистый ACK без `smsc_message_id` на проводе, поэтому в ленте одного сообщения видны 3 из 4 PDU (сам ACK реально пишется в ClickHouse, просто не коррелируется по конкретному сообщению).

**P2A: ❌ по-прежнему архитектурно не существует.** Сообщения от абонента к платформе (не наоборот) — весь пайплайн сегодня спроектирован под A2P-направление; `deliver_sm` безусловно трактуется как DLR (`esm_class` уже декодируется кодеком, но не используется для различения MO/DLR — реальный MO сегодня тихо сломал бы DLR-парсинг). **~1-2 недели отдельной фазой**, не между делом — фазовый план (дизайн, код не писать до отдельного подтверждения) зафиксирован в `BACKOFFICE_ROADMAP.md`, раздел "P2A (входящие MO...)".
...
| 14 | ~~Пер-PDU SMPP таймстампы~~ ✅ готово (см. `BACKOFFICE_ROADMAP.md`) | 2-3 дня | нет, только детализация |
...
| 19 | ~~Chat админ↔партнёр (экран 27)~~ ✅ готово (обе стороны, живой раунд-трип проверен) | реальная новая фича, новый `chat-service` | нет |
...
- Chat админ↔партнёр (экран 27) — подтверждён нужным. Реальная новая фича с нуля (хранилище + доставка + два UI) — отдельный заход, не между делом. **✅ реализовано**: новый `chat-service` (см. раздел "Экран 27" выше), обе стороны (`backoffice-ui`/`partner-portal-ui`), живой polling-раунд-трип проверен docker-compose.
...
- A2P/DLR per-PDU логи (экраны 38-40, первая половина) — подтверждены нужными именно как в референсе. **✅ реализовано** (см. раздел выше и `BACKOFFICE_ROADMAP.md`), живьём проверено сквозным реальным сообщением. P2A (вторая половина той же пары экранов) остаётся отдельным, ещё не начатым куском — фазовый план (дизайн-онли) в `BACKOFFICE_ROADMAP.md`.
