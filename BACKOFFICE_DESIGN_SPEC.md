...
- Regex Patterns (экран 36) → переименован "Pattern Placeholders" — именованные переменные с regex для использования внутри шаблонов. Не 0-backend: требует изменения движка матчинга `policy-service` (`template_matching.rs`), не аддитивная таблица — отложено, отдельный заход.
