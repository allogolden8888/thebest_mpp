...
**Статус: ⚠️ обсудить, нужно ли вообще.** CTN (termination number) — ноль совпадений в кодовой базе, чужая для текущей модели маршрутизации концепция (в этой платформе маршрут определяется `operator_id`/`route_table`, не отдельным биллинговым объектом terminaton-number). Прежде чем оценивать объём — решить, действительно ли платформе нужна эта сущность, или это калька с чужого референса без реального применения здесь.
...
**Статус: ⚠️ обсудить.** Сегодня категория (`SERVICE`/`TRANSACTION`/`ADVERTISING`/`UNTEMPLATED`/`BLOCKED`) выводится из совпавшего `policy_template`, отдельной regex-per-category сущности с собственным CRUD нет. Прежде чем проектировать новую таблицу/`entity_type` — решить, это то же самое, что шаблоны другими словами (тогда 0 новой работы, полировка UI), или реально отдельная сущность (тогда архитектурное решение, не эндпоинт).
...
**Статус: ⚠️ обсудить.** Общий именованный реестр (name + pattern + "используется в") — новая сущность, но небольшая. Прежде чем строить отдельный CRUD — решить, действительно ли нужен отдельно от `policy_template`/`policy_ruleset`, или это тот же паттерн, что уже есть, под другим UI.
...
**Требуют вашего решения перед дизайном:**
- MO/P2A — нужны сейчас или позже?
- Пер-PDU детализация SMPP (как на 2-м скрине) — нужна?
- Тариф per-operator — нужен?
- Поиск сообщений по номеру абонента — нужен? (потребует индекс/проекцию)
- CTN (экран 23) — эта концепция вообще нужна платформе, или это калька с чужого референс-дизайна без реального применения здесь?
- Categories (экран 32) — то же самое, что `policy_template.category` другими словами, или реально отдельная сущность со своим regex/CDR-флагом?
- Spam Patterns (экран 37) — то же самое, что уже существующие banwords/antispam (`policy_ruleset`, Экран 10, уже полностью функционален), или другая семантика?
- Regex Patterns (экран 36) — отдельный именованный реестр нужен, или Categories/Spam Patterns могут переиспользовать `policy_template`/`policy_ruleset` без него?
- Admin users (экран 33) — реально нужна LDAP/Active Directory интеграция, или достаточно уже существующего списка сотрудников с ролями (Экран 16)?
...
- Chat админ↔партнёр (экран 27) — насколько это приоритетно; это реальная новая фича с нуля (хранилище + доставка + два UI), не переиспользование существующих данных?
