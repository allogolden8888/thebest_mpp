...
**Расширение 5.1 (development_plan.md) — добавлено по общему знанию нумерации Узбекистана, НЕ проверено на реальных номерах (в отличие от таблицы выше):**
...
Добавлены только для операторов, у которых в платформе уже есть реальный connection-профиль (`config_schemas/examples/operator.valid.json` и т.д.) — намеренно не добавлены другие узбекистанские операторы (Perfectum как отдельный от Uzmobile бренд, Mobiuz и другие более новые), т.к. для них нет connection-профиля нигде в репозитории: запись number_range без соответствующего оператора резолвила бы сообщение туда, куда его физически нечем доставить. Требует полевой проверки на реальных номерах той же степени, что и таблица выше, прежде чем считаться окончательным.
